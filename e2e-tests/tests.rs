use e2e_tests::localstack::{setup_s3, LocalStack};
...
impl Setup {
    pub async fn new() -> Self {
        ...
        // Setup S3
        setup_s3(localstack_instance.s3_client).await.unwrap();
...
async fn test_orchestrator_workflow() {
    ...
    let test_result =
        wait_for_db_state(Duration::from_secs(1200), l2_block_number, setup_config.mongo_db_instance()).await;
    assert!(test_result.is_ok(), "Final DB state assertion failed.");
}
...
async fn wait_for_db_state(
    ...
    let expected_state = ExpectedDBState {
        internal_id: l2_block_for_testing.clone(),
        external_id: ExternalId::String(Box::from(l2_block_for_testing.clone())),
    ...
    while start.elapsed() < timeout {
        let db_state = get_database_state(mongo_db_server, l2_block_for_testing.clone()).await.unwrap();
        if db_state.is_some() && db_state.unwrap() == expected_state {
            return Ok(());
        }
        tokio::time::sleep(Duration::from_millis(100)).await;
    }

    Err(format!("Timed out waiting for expected state: {:?}", expected_state))
}
