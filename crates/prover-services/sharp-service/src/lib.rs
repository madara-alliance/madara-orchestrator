...
impl ProverClient for SharpProverService {
    async fn submit_task(&self, task: Task) -> Result<TaskId, ProverClientError> {
        match task {
            Task::CairoPie(cairo_pie) => {
                ...
                let encoded_pie = snos::sharp::pie::encode_pie_mem(cairo_pie)
                    .map_err(ProverClientError::PieEncoding)?;
                ...
                if let Some(job_key) = res.cairo_job_key {
                    ...
                    Err(ProverClientError::TaskInvalid(
                        res.error_message.unwrap_or_default(),
                    ))
                }
            }
        }
    ...
    async fn get_task_status(&self, task_id: &TaskId) -> Result<TaskStatus, ProverClientError> {
        ...
        match res.status {
            ...
            CairoJobStatus::INVALID => Ok(TaskStatus::Failed(format!(
                "Task is invalid: {:?}",
                res.invalid_reason.unwrap_or_default()
            ))),
            CairoJobStatus::UNKNOWN => {
                Ok(TaskStatus::Failed(format!("Task not found: {}", task_id)))
            }
            CairoJobStatus::IN_PROGRESS
            | CairoJobStatus::NOT_CREATED
            | CairoJobStatus::PROCESSED => Ok(TaskStatus::Processing),
            CairoJobStatus::ONCHAIN => {
                if self.fact_checker.is_valid(&fact).await? {
                    Ok(TaskStatus::Succeeded)
                } else {
                    Ok(TaskStatus::Failed(format!(
                        "Fact {} is not valid or not registed",
                        hex::encode(fact)
                    )))
                }
            }
