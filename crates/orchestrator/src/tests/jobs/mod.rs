...
mod job_handler_tests {
    ...
    #[rstest]
    #[tokio::test]
    async fn create_job_handler_job_does_not_exists_in_db() {
        ...
        let consumed_messages =
            config.queue().consume_message_from_queue(JOB_PROCESSING_QUEUE.to_string()).await.unwrap();
    ...
    #[rstest]
    #[tokio::test]
    async fn create_job_handler_job_exists_in_db() {
        ...
        assert!(matches!(consumed_messages, QueueError::NoData));
    }
    ...
    #[tokio::test]
    async fn create_job_handler_job_handler_is_not_implemented() {
        TestConfigBuilder::new().build().await;
        let config = config().await;
    ...
    #[tokio::test]
    async fn process_job_handler_job_exists_in_db_and_valid_job_processing_status(
        ...
        let consumed_message_payload: MessagePayloadType = consumed_messages.payload_serde_json().unwrap().unwrap();
        println!("payload: {:?}", consumed_message_payload);
        assert_eq!(consumed_message_payload.id, job_item.id);
    }
    ...
    #[rstest]
    #[tokio::test]
    async fn process_job_handler_job_exists_in_db_and_invalid_job_processing_status() {
    ...
    #[rstest]
    #[tokio::test]
    async fn process_job_two_workers_process_same_job() {
        let mut job_handler = MockJob::new();
        // Expecting process job function in job processor to return the external ID.
        job_handler.expect_process_job().times(1).returning(move |_, _| Ok("0xbeef".to_string()));
        job_handler.expect_verification_polling_delay_seconds().return_const(1u64);
    ...
    async fn verify_job_handler_with_expected_job_status_and_verified_status_return() {
        ...
        let consumed_messages_verification_queue =
            config.queue().consume_message_from_queue(JOB_VERIFICATION_QUEUE.to_string()).await.unwrap_err();
        assert!(matches!(consumed_messages_verification_queue, QueueError::NoData));
        let consumed_messages_processing_queue =
            config.queue().consume_message_from_queue(JOB_PROCESSING_QUEUE.to_string()).await.unwrap_err();
        assert!(matches!(consumed_messages_processing_queue, QueueError::NoData));
    }
    ...
    #[rstest]
    #[tokio::test]
    async fn verify_job_handler_with_expected_job_status_and_rejected_status_return_and_adds_process_to_job_queue() {
        let job_item =
            build_job_item_by_type_and_status(JobType::DataSubmission, JobStatus::PendingVerification, "1".to_string());
    ...
    #[rstest]
    #[tokio::test]
    async fn verify_job_handler_with_expected_job_status_and_rejected_status_return() {
        let mut job_item =
            build_job_item_by_type_and_status(JobType::DataSubmission, JobStatus::PendingVerification, "1".to_string());
    ...
    #[rstest]
    #[tokio::test]
    async fn verify_job_handler_with_expected_job_status_and_pending_status_return() {
        let mut job_item =
            build_job_item_by_type_and_status(JobType::DataSubmission, JobStatus::PendingVerification, "1".to_string());
